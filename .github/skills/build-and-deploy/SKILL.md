...
description: Build, deploy, and verify changes for Kiri CRM. Use this skill after making any code change to compile, deploy, and validate that everything works correctly. Covers Docker-based builds for Go backend and Next.js frontend.
---

# Build and Deploy — Kiri CRM
...
### Backend (Go/Fiber)
```bash
cd /root/proyect/kiri && docker compose build backend
```

### Frontend (Next.js/React)
```bash
cd /root/proyect/kiri && docker compose build frontend
```

### Both services
```bash
cd /root/proyect/kiri && docker compose build backend frontend
```
...
```bash
cd /root/proyect/kiri && docker compose up -d
```
