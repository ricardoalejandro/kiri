...
1. **Backend (Go/Fiber):** Ejecutar `cd /root/proyect/kiri && docker compose build backend` y verificar que compila sin errores.
2. **Frontend (Next.js/React):** Ejecutar `cd /root/proyect/kiri && docker compose build frontend` y verificar que compila sin errores.
...
| Deploy | Dokploy | kiri.naperu.cloud |
...
backend/
  ...
  internal/
    ...
    kommo/                         → Integración con Kommo CRM
      ...
      sync.go                      → Worker de sincronización unidireccional Kommo→Kiri
