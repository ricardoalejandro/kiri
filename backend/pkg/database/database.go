...
func Migrate(db *pgxpool.Pool) error {
	...
	migrations := []string{
		...
		`ALTER TABLE users ADD COLUMN IF NOT EXISTS groq_api_key TEXT DEFAULT ''`,
		...
		// DNI and BirthDate on leads and contacts
		...
		`CREATE TABLE IF NOT EXISTS platform_announcements (
			...
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CHECK (type IN ('info', 'warning', 'update')),
		...
		`CREATE TABLE IF NOT EXISTS platform_announcement_reads (
			...
			UNIQUE (announcement_id, user_id, account_id)
		)`,
		`CREATE TABLE IF NOT EXISTS feedback_suggestions (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
			user_id UUID REFERENCES users(id) ON DELETE SET NULL,
			category VARCHAR(40) NOT NULL DEFAULT 'idea',
			...
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CHECK (category IN ('idea', 'problem', 'improvement', 'question')),
			CHECK (status IN ('new', 'reviewing', 'planned', 'resolved', 'closed'))
		)`,
		`CREATE INDEX IF NOT EXISTS idx_announcements_status_dates ON platform_announcements(status, starts_at, ends_at)`,
		`CREATE INDEX IF NOT EXISTS idx_announcement_targets_account ON platform_announcement_targets(account_id)`,
		`CREATE INDEX IF NOT EXISTS idx_announcement_reads_user_account ON platform_announcement_reads(user_id, account_id)`,
		`CREATE INDEX IF NOT EXISTS idx_feedback_account_created ON feedback_suggestions(account_id, created_at DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_feedback_status_created ON feedback_suggestions(status, created_at DESC)`,
