...
func getToolDefinitions() []groqTool {
	return []groqTool{
		...
		},
		{
			...
			Function: groqFunction{
				Name:        "run_query",
				Description: "Ejecuta consulta SQL SELECT read-only en PostgreSQL. $1=account_id. Máx 100 filas.",
				Parameters: map[string]interface{}{
					"type": "object",
					"properties": map[string]interface{}{
...
var sqlRetiredTablesPattern = regexp.MustCompile(`(?i)\b(events?|event_logbooks?|event_participants?|programs?|program_sessions?|program_participants?|surveys?|survey_questions?|dynamics?|dynamic_links?|kommo_[a-z_]*|api_keys|integration_instances?)\b`)
...
func (s *Server) toolRunQuery(ctx context.Context, args map[string]interface{}, accountID string) (interface{}, error) {
	...
	if strings.Contains(trimmed, ";") {
		return map[string]interface{}{"error": "No se permiten múltiples sentencias SQL."}, nil
	}
	if sqlRetiredTablesPattern.MatchString(trimmed) {
		return map[string]interface{}{"error": "Consulta referencia módulos retirados de Kiri."}, nil
	}
	if !strings.Contains(trimmed, "$1") || !strings.Contains(strings.ToLower(trimmed), "account_id") {
		return map[string]interface{}{"error": "Toda consulta debe filtrar por account_id usando $1."}, nil
	}
	...
	log.Printf("[AI-SQL] Executing scoped read-only query (account=%s)", accountID)
...
`, accountID)
	if err := row.Scan(
		&data.TotalLeads, &data.TotalContacts,
...
func buildSystemPrompt(crm *crmContextData, currentPage, customRole, customInstructions string) string {
	...
	sb.WriteString(fmt.Sprintf("• Leads totales: %d | Contactos WhatsApp: %d\n", crm.TotalLeads, crm.TotalContacts))
...
SCHEMA:event_logbooks(id uuid PK, event_id uuid FK→events, date date, title text, status text, total_participants int, general_notes text) -- SESIONESevent_participants(id uuid PK, event_id uuid FK→events, name text, phone text, email text, stage_id uuid, lead_id uuid, notes text, age int)
...
interactions(id uuid PK, account_id uuid, contact_id uuid, type text, direction text, outcome text, notes text, lead_id uuid, created_at timestamptz)
