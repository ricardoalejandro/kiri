...
func NewServer(cfg *config.Config, services *service.Services, repos *repository.Repositories, hub *ws.Hub, pool *whatsapp.DevicePool, store *storage.Storage, c *cache.Cache, gc *googleclient.Client, version string) *Server {
	...
	app.Use(func(c *fiber.Ctx) error {
		c.Set("X-Kiri-Version", server.version)
		return c.Next()
	})
...
func objectKeyFromMediaURL(mediaURL string) string {
	...
	if sidx := strings.Index(mediaURL, "/kiri-media/"); sidx >= 0 {
		return mediaURL[sidx+len("/kiri-media/"):]
	}
	return ""
}
...
func (s *Server) handleCreateLead(c *fiber.Ctx) error {
	...
	if phone != "" {
		...
		jid = fmt.Sprintf("manual_%s@kiri.lead", uuid.New().String()[:8])
	}
...
func (s *Server) handleCreateContact(c *fiber.Ctx) error {
	...
	if normalizedPhone != "" {
		...
		jid = fmt.Sprintf("manual_%s@kiri.contact", uuid.New().String()[:8])
	}
...
func (s *Server) handleAdminUpdateUser(c *fiber.Ctx) error {
	...
	if strings.TrimSpace(req.Email) == "" && strings.TrimSpace(req.Username) != "" {
		req.Email = fmt.Sprintf("%s@users.kiri.local", strings.ToLower(strings.TrimSpace(req.Username)))
	}
	if req.Role == "" {
